...
# Full stock-analysis documents include chart history, so their Firestore
# payloads reach the 10 MiB commit limit well before the 500-write limit.
FIRESTORE_BATCH_SIZE = 40

# Summary-only meta documents remain small enough to keep the larger chunks.
META_CHUNK_SIZE = 400
...
class StockAnalyzer:
    ...
    def sync_meta_data_snapshot(self, market: str, summary_all: List[Dict[str, Any]], previous_doc_count: int = 0) -> int:
        ...
        for start in range(0, len(summary_all), META_CHUNK_SIZE):
            chunk = summary_all[start : start + META_CHUNK_SIZE]
