...
- React + Vite 프론트엔드 (Firebase Auth/Firestore 연동)
- Cloud Functions 스케줄 함수
  - 함수명: `refreshPendingOrdersFromBuySignals`
  - 스케줄: 평일 15:40 (Asia/Seoul)
  - 동작: `stock_analysis`의 `buy_signal`만 조회해 `pending_orders` 갱신
- 로컬 비상 실행 Python 스크립트
  - 파일: `functions/local_refresh_pending_orders.py`
  - 용도: 스케줄 함수 장애 시 수동 갱신
